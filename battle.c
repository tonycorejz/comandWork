...
int battle(struct fields field)
{
	...
	while(1){
		 ...
		 */
		printf("\nTBoe :");
		scanf("%d %d", &i_h, &j_h); //функция вызова координат 
		
		switch(field.opponent_field[i_h][j_h]){
			case 0:
				//Функция выставления точки 
				field.opponent_field[i_h][j_h] = 3;
				break;

			case 1:
				//Отрисовка что выстрел
				field.opponent_field[i_h][j_h] = 2;
				break;
		...
		if((check_opponent = check(field.opponent_field)) == 0)
			return 0; //Передаем что победили
		else{
			//Ход противника
			 ...
			 */
		printf("\nBrag : ");
		scanf("%d %d", &i_h, &j_h); //функция получения координат от противника 
		switch(field.my_field[i_h][j_h]){
			case 0:
				//Функция выставления точки 
				field.my_field[i_h][j_h] = 3;
				break;

			case 1:
				//Отрисовка что выстрел
				field.my_field[i_h][j_h] = 2;
				break;
